...
df["Genres Extracted"] = df["Genres Extracted"].apply(
    eval if isinstance(df["Genres Extracted"].iloc[0], str) else lambda x: x
)
...
viz_option = st.selectbox(
    "Choose a plot", ["Genre Distribution", "Rating Distribution", "Top Rated Books"]
)

st.write(
    "Visualizations help in understanding the distribution "
